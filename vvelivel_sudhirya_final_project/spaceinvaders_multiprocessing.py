...
class PPO:
    def __init__(self, sharedpolicy, sharedoptimizer, state_dim, action_dim, n_latent_var, lr, betas, gamma, K_epochs, eps_clip):
        self.lr = lr
        self.betas = betas
        self.gamma = gamma
        ...
        self.sharedpolicy = sharedpolicy
        self.sharedoptimizer = sharedoptimizer
        self.policy = ActorCritic(state_dim, action_dim, n_latent_var)
        self.policy_old = ActorCritic(state_dim, action_dim, n_latent_var)
        self.policy_old.load_state_dict(self.sharedpolicy.state_dict())
    ...
    def update(self, memory):   
        ...
        self.policy.load_state_dict(self.sharedpolicy.state_dict())
        ...
        for start in range(0, steps, self.minibatch_steps): #range(self.K_epochs):
            ...
            # take gradient step
            self.sharedoptimizer.zero_grad()
            loss.mean().backward()
            torch.nn.utils.clip_grad_norm(self.policy.parameters(), self.max_grad_norm)
            for param, shared_param in zip(self.policy.parameters(), self.sharedpolicy.parameters()):
                if shared_param.grad is None: shared_param._grad = param.grad # sync gradients with shared model
            self.sharedoptimizer.step()
...
def train(sharedpolicy, sharedoptimizer, rank, state_dim, action_dim, n_latent_var, lr, betas, gamma, K_epochs, eps_clip, max_episodes, max_timesteps, log_interval, env_name):
    game = Game(112+ rank)
    torch.manual_seed(112 + rank)
    memory = Memory()
    ppo = PPO(sharedpolicy, sharedoptimizer, state_dim, action_dim, n_latent_var, lr, betas, gamma, K_epochs, eps_clip)
    print(lr,betas)
    ...
    for i_episode in range(1, max_episodes+1):
        ...
        if rank == 0 and running_reward > (highest_running_reward):
            #print("########## Solved! ##########")
            highest_running_reward = running_reward
            torch.save(ppo.sharedpolicy.state_dict(), './PPO_{}_mp.pth'.format(env_name))
            #break
            
        # logging
        if rank==0  and i_episode % log_interval == 0:
            avg_length = int(avg_length/log_interval)
            running_reward = int((running_reward/log_interval))
            
            print('Episode {} \t avg length: {} \t reward: {}'.format(i_episode, avg_length, running_reward))
            all_running_rewards.append(running_reward)
            myplt(all_running_rewards, lbl=f'{env_name}_Reward_mp')
...
def main():
    ...
    sharedpolicy = ActorCritic(state_dim, action_dim, n_latent_var).share_memory()
    sharedoptimizer = SharedAdam(sharedpolicy.parameters(), lr=lr)
